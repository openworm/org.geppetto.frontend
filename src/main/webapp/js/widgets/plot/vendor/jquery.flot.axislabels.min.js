...
*GEPPETTO HACK*
Modified for Geppetto to display labels in the right place when axis are hidden (show: false)
 */
(function ($) {
    var options = { };
        ...
        plot.hooks.draw.push(function (plot, ctx) {
            if (!secondPass) {
                ...
                $.each(plot.getAxes(), function(axisName, axis) {
                    ...
                    // **GEPPETTO HACK**
                    // Set a flag to indicate the axis is hidden
                    if(axis.box == undefined && axisName === 'xaxis')
                    {
                    	axis.hidden = true;
                    }
                    
                    // If the axis is hidden, every time we draw we simulate an axis box to force label position
                    if(axis.hidden === true){
                    	var width = 0;
                    	var height = 0;
                    	var top = parseInt($(plot.getCanvas()).css("height").replace("px","")) + 17.5;
                    	var left = parseInt($(plot.getCanvas()).css("width").replace("px",""))/3;
                    	
                    	axis.box = {height: height, left: left, padding: 5, top: top, width: left};
                    	axisLabels[axisName].position = "bottom";
                    	axisLabels[axisName].requiresResize=true;
                    }
                    // **GEPPETTO HACK**
